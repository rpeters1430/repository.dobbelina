...
import types
def mock_kodi():
    if "kodi_six" in sys.modules: return
    
    # Mock xbmc modules
    for name in ["xbmc", "xbmcgui", "xbmcplugin", "xbmcvfs", "xbmcaddon", "StorageServer", "CommonFunctions", "requests", "websocket"]:
        mock = types.ModuleType(name)
        if name == "requests":
            mock.Session = lambda: types.SimpleNamespace(get=lambda *args, **kwargs: None, headers={})
        ...
        if name == "xbmcgui":
            ...
            class ListItem:
                ...
                def addContextMenuItems(self, *args): pass
...
class SmokeRunner:
    ...
    def run_site_test(self, site_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if not site_obj:
                return {"site": site_name, "status": "ERROR", "message": "No 'site' object found in module"}
        except Exception as e:
            return {"site": site_name, "status": "ERROR", "message": f"Import error: {str(e)}"}
        ...
        try:
            ...
            if html and len(html) > 500:
                if "Cloudflare" in html or "Just a moment" in html or "Verify you are human" in html:
                ...
                result["notes"].append(f"Standard fetch returned minimal content ({len(html) if html else 0} bytes).")
        except Exception as e:
            ...
            try:
                ...
                if pw_html and len(pw_html) > 500:
                    if "Cloudflare" in pw_html or "Just a moment" in pw_html:
                        ...
                        result["notes"].append("Accessible via Playwright (JS required).")
                else:
    ...
    def generate_report(self):
        ...
        with open(json_path, "w") as f:
            json.dump({"timestamp": self.timestamp, "results": self.results}, f, indent=2)
        ...
        pw_ok = len([r for r in self.results if r.get("status") == "OK (PLAYWRIGHT ONLY)"])
        ...
        for r in self.results:
            ...
            if r.get("message"):
                notes += f" {r['message']}"
            lines.append(f"| {r.get('site')} | {r.get('url')} | {r.get('standard_fetch')} | {r.get('playwright_fetch')} | {status_icon} {status} | {notes} |")
    ...
    def run_all(self, limit: Optional[int] = None):
        ...
        for site in all_sites:
            try:
                ...
                self.results.append({"site": site, "status": "CRASH", "message": str(e)})
