...
def process_logo(input_path, output_path):
    ...
    try:
        ...
        base_temp_output = Path(output_path).with_name(
            f"temp_std_{Path(output_path).name}"
        )
        ...
        potential_files = list(
            base_temp_output.parent.glob(f"{base_temp_output.stem}*")
        )
        if not potential_files:
            # Try specifically with -0 if base didn't exist
            potential_files = list(
                base_temp_output.parent.glob(f"{base_temp_output.stem}-0*")
            )
...
def create_placeholder_logo(display_name, output_path):
    ...
    try:
        ...
        base_temp_output = Path(output_path).with_name(
            f"temp_place_{Path(output_path).name}"
        )
