...
class SyncManager:
    ...
    def _load_git_history_hashes(self):
        ...
        hashes = set(re.findall(r"cherry picked from commit ([0-9a-f]{7,40})", result.stdout))
        return hashes
    ...
    def get_new_commits(self):
        ...
        result = self._run_git(["log", "upstream/master", "--not", "origin/master", "--oneline", "--no-merges"])
        commits = []
    ...
    def run(self):
        ...
        for c in new_commits:
            ...
            if sha in self.tracked_hashes or any(sha in h for h in self.tracked_hashes):
                continue
            # Check git history
            if sha in self.integrated_in_git or any(sha in h for h in self.integrated_in_git):
                continue
            pending.append(c)
        ...
        for c in pending:
            ...
            if analysis["is_bs4_only"]:
                print(f"⏭️  Auto-skipping {c['sha']} (BS4 site: {', '.join(analysis['sites'])})")
                to_skip.append(c)
            else:
        ...
        if to_skip:
            ...
            if confirm.lower() == 'y':
                for c in to_skip:
                    self.update_sync_file(c["sha"], c["msg"], skip_reason="Fork has BeautifulSoup migration")
