...
sys.modules["kodi_six.xbmc"].getInfoLabel = (
    lambda key: "20.0" if "BuildVersion" in key else ""
)
...
sys.modules["kodi_six.xbmcvfs"].translatePath = lambda p: str(p).replace(
    "special://profile", str(ROOT / ".profile")
)
sys.modules["xbmcvfs"].exists = lambda p: Path(str(p)).exists()
sys.modules["kodi_six.xbmcvfs"].exists = lambda p: Path(str(p)).exists()
sys.modules["xbmcvfs"].mkdirs = lambda p: Path(str(p)).mkdir(
    parents=True, exist_ok=True
)
sys.modules["kodi_six.xbmcvfs"].mkdirs = lambda p: Path(str(p)).mkdir(
    parents=True, exist_ok=True
)
...
def analyze_source_code(file_path: Path) -> Dict[str, Any]:
    ...
    uses_beautifulsoup = bool(re.search(r"\bparse_html\b|\bBeautifulSoup\b", source))
    uses_regex = bool(
...
def generate_report(sites_info: List[SiteInfo]) -> Dict[str, Any]:
    ...
    regex_only_sites = [
        s for s in successful if s.uses_regex and not s.uses_beautifulsoup
    ]
    ...
    return {
        ...
        "errors": [{"name": s.name, "error": s.import_error} for s in with_errors],
        "missing_critical": {
