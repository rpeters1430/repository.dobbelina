...
site_porndish = importlib.import_module("resources.lib.sites.porndish")

def test_porndish():
    print("Testing porndish...")
    site_url = "https://www.porndish.com/"
    
    # Test Listing
    print("\n--- Testing Listing ---")
    try:
        site_porndish.List(f"{site_url}page/1/")
    ...
    html, _ = utils.get_html_with_cloudflare_retry(f"{site_url}page/1/", site_url)
    if html:
        soup = utils.parse_html(html)
        item = soup.select_one("article a[title][href]")
        if item:
            video_url = utils.safe_get_attr(item, "href")
            print(f"Testing playback for: {video_url}")
            try:
                site_porndish.Playvid(video_url, "Test Video")
