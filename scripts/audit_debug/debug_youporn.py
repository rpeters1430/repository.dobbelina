...
site_youporn = importlib.import_module("resources.lib.sites.youporn")

def test_youporn():
    print("Testing youporn...")
    site_url = "https://www.youporn.com/"
    
    # Test Listing
    print("\n--- Testing Listing ---")
    try:
        site_youporn.List(f"{site_url}browse/time/")
    ...
    html = utils.getHtml(f"{site_url}browse/time/", site_url, {"Cookie": "age_verified=1"})
    soup = utils.parse_html(html)
    item = soup.select_one(".video-box")
    if item:
        ...
        try:
            site_youporn.Playvid(video_url, "Test Video")
