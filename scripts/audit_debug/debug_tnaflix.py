...
site_tnaflix = importlib.import_module("resources.lib.sites.tnaflix")

def test_tnaflix():
    print("Testing tnaflix...")
    site_url = "https://www.tnaflix.com/"
    
    # Test Listing
    print("\n--- Testing Listing ---")
    try:
        site_tnaflix.List(f"{site_url}new/1")
    ...
    html = utils.getHtml(f"{site_url}new/1", site_url)
    soup = utils.parse_html(html)
    item = soup.select_one('[data-vid]')
    if item:
        ...
        try:
            site_tnaflix.Playvid(video_url, "Test Video")
