...
site_fullporner = importlib.import_module("resources.lib.sites.fullporner")

def test_fullporner():
    print("Testing fullporner...")
    site_url = "https://fullporner.org/"
    
    # Test Listing
    print("\n--- Testing Listing ---")
    try:
        site_fullporner.List(f"{site_url}porno-channel/latest-xxx-videos/")
    ...
    html = utils.getHtml(f"{site_url}porno-channel/latest-xxx-videos/")
    ...
    if article:
        ...
        try:
            site_fullporner.Playvid(video_url, "Test Video")
