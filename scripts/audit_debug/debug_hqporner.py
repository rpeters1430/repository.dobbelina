...
site_hqporner = importlib.import_module("resources.lib.sites.hqporner")

def test_hqporner():
    print("Testing hqporner...")
    site_url = "https://hqporner.com/"
    
    # Test Listing
    print("\n--- Testing Listing ---")
    try:
        site_hqporner.HQLIST(f"{site_url}hdporn/1")
        print("HQLIST function executed successfully")
    except Exception as e:
        print(f"Error in HQLIST: {e}")

    # Test Playback Extraction
    print("\n--- Testing Playback Extraction ---")
    html = utils.getHtml(f"{site_url}hdporn/1")
    soup = utils.parse_html(html)
    item = soup.select_one("section.box.feature")
    if item:
        link = item.select_one("a.image.featured")
        ...
        try:
            site_hqporner.HQPLAY(video_url, "Test Video")
            print("HQPLAY function executed successfully")
        except Exception as e:
            print(f"Error in HQPLAY: {e}")
    else:
