...
site_hentaihavenco = importlib.import_module("resources.lib.sites.hentaihavenco")

def test_hentaihavenco():
    print("Testing hentaihavenco...")
    site_url = "https://hentaihaven.co/"
    
    # Test Listing
    print("\n--- Testing Listing ---")
    try:
        site_hentaihavenco.List(site_url)
    ...
    print("\n--- Testing Categories ---")
    try:
        site_hentaihavenco.Categories(f"{site_url}genres/")
    ...
    html = utils.getHtml(site_url)
    soup = utils.parse_html(html)
    item = soup.select_one("a.a_item")
    if item:
        ...
        try:
            site_hentaihavenco.Playvid(video_url, "Test Video")
