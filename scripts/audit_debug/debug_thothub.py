...
site_thothub = importlib.import_module("resources.lib.sites.thothub")

def test_thothub():
    print("Testing thothub...")
    site_url = "https://thothub.mx/"
    
    # Test Listing
    print("\n--- Testing Listing ---")
    try:
        site_thothub.List(f"{site_url}public/")
    ...
    html = utils.getHtml(f"{site_url}public/", site_url)
    soup = utils.parse_html(html)
    item = soup.select_one('.item a[href*="/videos/"]')
    if item:
        ...
        try:
            site_thothub.Playvid(video_url, "Test Video")
