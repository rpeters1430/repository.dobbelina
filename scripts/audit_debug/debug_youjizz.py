...
site_youjizz = importlib.import_module("resources.lib.sites.youjizz")

def test_youjizz():
    print("Testing youjizz...")
    site_url = "https://www.youjizz.com/"
    
    # Test Listing
    print("\n--- Testing Listing ---")
    try:
        site_youjizz.List(f"{site_url}newest-clips/1.html")
    ...
    html = utils.getHtml(f"{site_url}newest-clips/1.html", site_url, {"Cookie": "age_verified=1"})
    soup = utils.parse_html(html)
    item = soup.select_one(".video-thumb")
    if item:
        ...
        try:
            site_youjizz.Playvid(video_url, "Test Video")
