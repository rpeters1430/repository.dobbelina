...
site_spankbang = importlib.import_module("resources.lib.sites.spankbang")

def test_spankbang():
    print("Testing spankbang...")
    site_url = "https://spankbang.party/"
    
    # Test Listing
    print("\n--- Testing Listing ---")
    try:
        site_spankbang.List(f"{site_url}new_videos/1/")
    ...
    html = utils.getHtml(f"{site_url}new_videos/1/", site_url)
    soup = utils.parse_html(html)
    item = soup.select_one('[data-testid="video-item"]')
    if item:
        ...
        try:
            site_spankbang.Playvid(video_url, "Test Video")
