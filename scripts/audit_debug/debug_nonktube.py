...
site_nonktube = importlib.import_module("resources.lib.sites.nonktube")

def test_nonktube():
    print("Testing nonktube...")
    site_url = "https://www.nonktube.com/"
    
    # Test Listing
    print("\n--- Testing Listing ---")
    try:
        site_nonktube.List(f"{site_url}?sorting=latest")
    ...
    html = utils.getHtml(f"{site_url}?sorting=latest", site_url)
    soup = utils.parse_html(html)
    item = soup.select_one(".video-block")
    if item:
        link = item.select_one("a[href]")
        video_url = utils.safe_get_attr(link, "href")
        print(f"Testing playback for: {video_url}")
        try:
            site_nonktube.Playvid(video_url, "Test Video")
