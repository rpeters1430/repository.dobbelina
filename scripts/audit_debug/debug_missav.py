...
site_missav = importlib.import_module("resources.lib.sites.missav")

def test_missav():
    print("Testing missav...")
    site_url = "https://missav.ws/"
    
    # Test Listing
    print("\n--- Testing Listing ---")
    try:
        site_missav.List(f"{site_url}en/new?page=1")
    ...
    html, _ = utils.get_html_with_cloudflare_retry(f"{site_url}en/new?page=1", site_url)
    if html:
        soup = utils.parse_html(html)
        item = soup.select_one("a[href][alt]")
        if item:
            video_url = utils.safe_get_attr(item, "href")
            print(f"Testing playback for: {video_url}")
            try:
                site_missav.Playvid(video_url, "Test Video")
