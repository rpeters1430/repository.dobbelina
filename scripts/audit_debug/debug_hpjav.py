...
def test_hpjav():
    print("Testing hpjav...")
    site_url = "https://hpjav.in/"
    
    # Test Listing
    print("\n--- Testing Listing ---")
    try:
        site_hpjav.List(f"{site_url}trend/")
    ...
    html, _ = utils.get_html_with_cloudflare_retry(f"{site_url}trend/")
    soup = utils.parse_html(html)
    
    video_url = None
    all_links = soup.select('a[href*="/censored/"], a[href*="/uncensored/"]')
    ...
    if video_url:
        ...
        try:
            site_hpjav.Playvid(video_url, "Test Video")
