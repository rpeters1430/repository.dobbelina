...
site_sxyprn = importlib.import_module("resources.lib.sites.sxyprn")

def test_sxyprn():
    print("Testing sxyprn...")
    site_url = "https://sxyprn.com/"
    
    # Test Listing
    print("\n--- Testing Listing ---")
    try:
        site_sxyprn.List(f"{site_url}blog/all/0.html?fl=all&sm=latest")
    ...
    html = utils.getHtml(f"{site_url}blog/all/0.html?fl=all&sm=latest", site_url)
    soup = utils.parse_html(html)
    item = soup.select_one("div.post_el_small")
    if item:
        ...
        try:
            site_sxyprn.Playvid(video_url, "Test Video")
