...
# Try to install pycryptodome if missing (it might fail depending on environment)
try:
    from Cryptodome.Cipher import AES
except ImportError:
    print("Warning: Cryptodome missing. Playback test might fail for some hosts.")

site_premiumporn = importlib.import_module("resources.lib.sites.premiumporn")

def test_premiumporn():
    print("Testing premiumporn...")
    site_url = "https://premiumporn.org/"
    
    # Test Listing
    print("\n--- Testing Listing ---")
    try:
        site_premiumporn.List(f"{site_url}video/page/1/?rawx_per_page=24")
    ...
    html = utils.getHtml(f"{site_url}video/page/1/?rawx_per_page=24", site_url)
    ...
    if item:
        ...
        try:
            site_premiumporn.Play(video_url, "Test Video")
