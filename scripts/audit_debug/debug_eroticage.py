...
site_eroticage = importlib.import_module("resources.lib.sites.eroticage")

def test_eroticage():
    print("Testing eroticage...")
    site_url = "https://www.eroticage.net/"
    
    # Test Listing
    print("\n--- Testing Listing ---")
    try:
        site_eroticage.List(f"{site_url}?filter=latest")
    ...
    print("\n--- Testing Categories ---")
    try:
        site_eroticage.Categories(f"{site_url}categories/")
    ...
    article = soup.select_one("article[data-video-id]")
    if article:
        link = article.select_one("a[href]")
        video_url = utils.safe_get_attr(link, "href")
        print(f"Testing playback for: {video_url}")
        try:
            site_eroticage.Playvid(video_url, "Test Video")
