...
site_hornyfap = importlib.import_module("resources.lib.sites.hornyfap")

def test_hornyfap():
    print("Testing hornyfap...")
    site_url = "https://hornyfap.tv/"
    
    # Test Listing
    print("\n--- Testing Listing ---")
    try:
        site_hornyfap.List(f"{site_url}latest-updates/")
    ...
    print("\n--- Testing Categories ---")
    try:
        site_hornyfap.Categories(f"{site_url}categories/")
    ...
    html = utils.getHtml(f"{site_url}latest-updates/")
    ...
    if item:
        ...
        try:
            site_hornyfap.Playvid(video_url, "Test Video")
