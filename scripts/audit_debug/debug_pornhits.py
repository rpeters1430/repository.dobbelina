...
site_pornhits = importlib.import_module("resources.lib.sites.pornhits")

def test_pornhits():
    print("Testing pornhits...")
    site_url = "https://www.pornhits.com/"
    
    # Test Listing
    print("\n--- Testing Listing ---")
    try:
        site_pornhits.List(f"{site_url}videos.php?p=1&s=l")
    ...
    html = utils.getHtml(f"{site_url}videos.php?p=1&s=l", site_url)
    soup = utils.parse_html(html)
    item = soup.select_one("article.item")
    if item:
        ...
        try:
            site_pornhits.Playvid(video_url, "Test Video")
