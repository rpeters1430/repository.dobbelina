...
site_eporner = importlib.import_module("resources.lib.sites.eporner")

def test_eporner():
    print("Testing eporner...")
    site_url = "https://www.eporner.com/"
    
    # Test Listing
    print("\n--- Testing Listing ---")
    try:
        site_eporner.List(f"{site_url}recent/")
    ...
    print("\n--- Testing Categories ---")
    try:
        site_eporner.Categories(f"{site_url}cats/")
    ...
    html = utils.getHtml(f"{site_url}recent/")
    soup = utils.parse_html(html)
    video_item = soup.select_one("[data-vp] .mbcontent a")
    if video_item:
        video_url = utils.safe_get_attr(video_item, "href")
        if not video_url.startswith("http"):
             video_url = site_url[:-1] + video_url
        print(f"Testing playback for: {video_url}")
        try:
            site_eporner.Playvid(video_url, "Test Video")
