...
site_nltubes = importlib.import_module("resources.lib.sites.nltubes")

def test_nltubes():
    print("Testing nltubes (poldertube)...")
    site_url = "https://www.poldertube.nl/"
    
    # Test Listing
    print("\n--- Testing Listing ---")
    try:
        site_nltubes.NLVIDEOLIST(f"{site_url}?filter=latest")
        print("NLVIDEOLIST function executed successfully")
    except Exception as e:
        print(f"Error in NLVIDEOLIST: {e}")

    # Test Playback Extraction
    print("\n--- Testing Playback Extraction ---")
    html = utils.getHtml(f"{site_url}?filter=latest", "")
    soup = utils.parse_html(html)
    item = soup.select_one("article a[href][title]")
    if item:
        ...
        try:
            site_nltubes.NLPLAYVID(video_url, "Test Video")
            print("NLPLAYVID function executed successfully")
        except Exception as e:
            print(f"Error in NLPLAYVID: {e}")
    else:
