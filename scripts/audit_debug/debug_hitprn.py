...
site_hitprn = importlib.import_module("resources.lib.sites.hitprn")

def test_hitprn():
    print("Testing hitprn...")
    site_url = "https://www.hitprn.com/"
    
    # Test Listing
    print("\n--- Testing Listing ---")
    try:
        site_hitprn.List(f"{site_url}page/1/?orderby=date")
    ...
    html = utils.getHtml(f"{site_url}page/1/?orderby=date")
    soup = utils.parse_html(html)
    item = soup.select_one(".clip-link")
    if item:
        video_url = utils.safe_get_attr(item, "href")
        print(f"Testing playback for: {video_url}")
        try:
            site_hitprn.Play(video_url, "Test Video")
