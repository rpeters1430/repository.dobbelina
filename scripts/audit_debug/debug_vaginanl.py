...
site_vaginanl = importlib.import_module("resources.lib.sites.vaginanl")

def test_vaginanl():
    print("Testing vaginanl...")
    site_url = "https://vagina.nl/"
    
    # Test Listing
    print("\n--- Testing Listing ---")
    try:
        site_vaginanl.List(f"{site_url}sexfilms/newest")
    ...
    html = utils.getHtml(f"{site_url}sexfilms/newest", "")
    soup = utils.parse_html(html)
    item = soup.select_one(".card")
    if item:
        link = item.select_one("a.thumbnail-link[href]")
        ...
        try:
            site_vaginanl.Playvid(video_url, "Test Video")
