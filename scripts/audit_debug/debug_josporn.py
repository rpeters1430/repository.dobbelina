...
site_josporn = importlib.import_module("resources.lib.sites.josporn")

def test_josporn():
    print("Testing josporn...")
    site_url = "https://josporn.club/"
    
    # Test Listing
    print("\n--- Testing Listing ---")
    try:
        site_josporn.List(f"{site_url}videos/")
    ...
    html, _ = utils.get_html_with_cloudflare_retry(f"{site_url}videos/", site_url)
    soup = utils.parse_html(html)
    item = soup.select_one(".innercont")
    if item:
        link = item.select_one(".preview_screen a")
        ...
        try:
            site_josporn.Playvid(video_url, "Test Video")
