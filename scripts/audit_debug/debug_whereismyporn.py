...
site_whereismyporn = importlib.import_module("resources.lib.sites.whereismyporn")

def test_whereismyporn():
    print("Testing whereismyporn...")
    site_url = "https://whereismyporn.com/"
    
    # Test Listing
    print("\n--- Testing Listing ---")
    try:
        site_whereismyporn.List(f"{site_url}page/1")
    ...
    if item:
        link = item.select_one(".entry-title a[href]") or item.select_one("a[href]")
        video_url = utils.safe_get_attr(link, "href")
        print(f"Testing playback for: {video_url}")
        try:
            site_whereismyporn.Playvid(video_url, "Test Video")
