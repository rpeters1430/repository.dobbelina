...
site_reallifecam = importlib.import_module("resources.lib.sites.reallifecam")

def test_reallifecam():
    print("Testing reallifecam...")
    site_url = "https://reallifecam.to/"
    
    # Test Listing
    print("\n--- Testing Listing ---")
    try:
        site_reallifecam.List(f"{site_url}videos?o=mr")
    ...
    html = utils.getHtml(f"{site_url}videos?o=mr", site_url)
    soup = utils.parse_html(html)
    item = soup.select_one(".video-item, .item, .col-sm-6")
    if item:
        ...
        try:
            site_reallifecam.Playvid(video_url, "Test Video")
