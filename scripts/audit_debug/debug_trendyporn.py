...
site_trendyporn = importlib.import_module("resources.lib.sites.trendyporn")

def test_trendyporn():
    print("Testing trendyporn...")
    site_url = "https://www.trendyporn.com/"
    
    # Test Listing
    print("\n--- Testing Listing ---")
    try:
        site_trendyporn.List(f"{site_url}most-recent/")
    ...
    html = utils.getHtml(f"{site_url}most-recent/", site_url)
    soup = utils.parse_html(html)
    item = soup.select_one(".well.well-sm, .video-item, .item")
    if item:
        link = item.select_one("a.video-link, a[href]")
        video_url = utils.safe_get_attr(link, "href")
        print(f"Testing playback for: {video_url}")
        try:
            site_trendyporn.Playvid(video_url, "Test Video")
