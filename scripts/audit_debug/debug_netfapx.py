...
site_netfapx = importlib.import_module("resources.lib.sites.netfapx")

def test_netfapx():
    print("Testing netfapx...")
    site_url = "https://netfapx.com/"
    
    # Test Listing
    print("\n--- Testing Listing ---")
    try:
        site_netfapx.List(f"{site_url}?orderby=newest")
    ...
    html = utils.getHtml(f"{site_url}?orderby=newest", site_url)
    soup = utils.parse_html(html)
    item = soup.select_one("article.pinbox")
    if item:
        link = item.select_one("a[href]")
        video_url = utils.safe_get_attr(link, "href")
        print(f"Testing playback for: {video_url}")
        try:
            site_netfapx.Playvid(video_url, "Test Video")
