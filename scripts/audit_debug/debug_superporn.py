...
site_superporn = importlib.import_module("resources.lib.sites.superporn")

def test_superporn():
    print("Testing superporn...")
    site_url = "https://www.superporn.com/"
    
    # Test Listing
    print("\n--- Testing Listing ---")
    try:
        site_superporn.List(f"{site_url}videos/latest")
    ...
    html = utils.getHtml(f"{site_url}videos/latest", site_url)
    soup = utils.parse_html(html)
    item = soup.select_one(".thumb-video")
    if item:
        link = item.select_one("a.thumb-video__description")
        ...
        try:
            site_superporn.Playvid(video_url, "Test Video")
