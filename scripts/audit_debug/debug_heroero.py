...
site_heroero = importlib.import_module("resources.lib.sites.heroero")

def test_heroero():
    print("Testing heroero...")
    site_url = "https://heroero.com/"
    
    # Test Listing
    print("\n--- Testing Listing ---")
    try:
        site_heroero.List(f"{site_url}latest-updates/")
    ...
    print("\n--- Testing Categories ---")
    try:
        site_heroero.Categories(f"{site_url}categories/")
    ...
    if item:
        ...
        try:
            site_heroero.Playvid(video_url, "Test Video")
