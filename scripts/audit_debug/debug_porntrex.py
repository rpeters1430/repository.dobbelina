...
site_porntrex = importlib.import_module("resources.lib.sites.porntrex")

def test_porntrex():
    print("Testing porntrex...")
    site_url = "https://www.porntrex.com/"
    
    # Test Listing
    print("\n--- Testing Listing ---")
    try:
        site_porntrex.PTList(f"{site_url}latest-updates/")
        print("PTList function executed successfully")
    except Exception as e:
        print(f"Error in PTList: {e}")

    # Test Playback Extraction
    print("\n--- Testing Playback Extraction ---")
    html = utils.getHtml(f"{site_url}latest-updates/", site_url)
    soup = utils.parse_html(html)
    item = soup.select_one(".video-preview-screen")
    if item:
        link = item.select_one("a.thumb[href]")
        video_url = utils.safe_get_attr(link, "href")
        print(f"Testing playback for: {video_url}")
        try:
            site_porntrex.PTPlayvid(video_url, "Test Video")
            print("PTPlayvid function executed successfully")
        except Exception as e:
            print(f"Error in PTPlayvid: {e}")
    else:
