...
site_pornhub = importlib.import_module("resources.lib.sites.pornhub")

def test_pornhub():
    print("Testing pornhub...")
    site_url = "https://www.pornhub.com/"
    
    # Test Listing
    print("\n--- Testing Listing ---")
    try:
        site_pornhub.List(f"{site_url}video?o=cm")
    ...
    html = utils.getHtml(f"{site_url}video?o=cm", site_url, {"Cookie": "accessAgeDisclaimerPH=1; accessAgeDisclaimerUK=1"})
    soup = utils.parse_html(html)
    item = soup.select_one('a[href*="/view_video.php?viewkey="]')
    if item:
        ...
        try:
            site_pornhub.Playvid(video_url, "Test Video")
