...
site_speedporn = importlib.import_module("resources.lib.sites.speedporn")

def test_speedporn():
    print("Testing speedporn...")
    site_url = "https://speedporn.net/"
    
    # Test Listing
    print("\n--- Testing Listing ---")
    try:
        site_speedporn.List(f"{site_url}xxxfree/")
    ...
    html, _ = utils.get_html_with_cloudflare_retry(f"{site_url}xxxfree/")
    soup = utils.parse_html(html)
    item = soup.select_one("a.thumb")
    if item:
        video_url = utils.safe_get_attr(item, "href")
        print(f"Testing playback for: {video_url}")
        try:
            site_speedporn.Playvid(video_url, "Test Video")
