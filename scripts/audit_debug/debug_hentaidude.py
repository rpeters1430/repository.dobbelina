...
site_hentaidude = importlib.import_module("resources.lib.sites.hentaidude")

def test_hentaidude():
    print("Testing hentaidude...")
    site_url = "https://hentaidude.xxx/"
    
    # Test Listing
    print("\n--- Testing Listing ---")
    try:
        site_hentaidude.List(f"{site_url}page/1/?m_orderby=latest")
    ...
    html = utils.getHtml(f"{site_url}page/1/?m_orderby=latest")
    soup = utils.parse_html(html)
    item = soup.select_one(".page-item-detail, .tab-thumb")
    if item:
        link = item.select_one("a[href]")
        video_url = utils.safe_get_attr(link, "href")
        print(f"Testing playback for: {video_url}")
        try:
            site_hentaidude.Playvid(video_url, "Test Video")
