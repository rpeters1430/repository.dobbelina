...
site_whoreshub = importlib.import_module("resources.lib.sites.whoreshub")

def test_whoreshub():
    print("Testing whoreshub...")
    site_url = "https://www.whoreshub.com/"
    
    # Test Listing
    print("\n--- Testing Listing ---")
    try:
        site_whoreshub.List(f"{site_url}latest-updates/")
    ...
    item = soup.select_one("div.box a.item")
    if item:
        video_url = utils.safe_get_attr(item, "href")
        print(f"Testing playback for: {video_url}")
        try:
            site_whoreshub.Playvid(video_url, "Test Video")
