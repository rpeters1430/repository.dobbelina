...
site_pornone = importlib.import_module("resources.lib.sites.pornone")

def test_pornone():
    print("Testing pornone...")
    site_url = "https://pornone.com/"
    
    # Test Listing
    print("\n--- Testing Listing ---")
    try:
        site_pornone.List(f"{site_url}newest/")
    ...
    html = utils.getHtml(f"{site_url}newest/", site_url)
    soup = utils.parse_html(html)
    item = soup.select_one("a.popbop, .video-item")
    if item:
        ...
        try:
            site_pornone.Playvid(video_url, "Test Video")
