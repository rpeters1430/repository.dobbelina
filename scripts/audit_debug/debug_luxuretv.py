...
site_luxuretv = importlib.import_module("resources.lib.sites.luxuretv")

def test_luxuretv():
    print("Testing luxuretv...")
    site_url = "https://en.luxuretv.com/"
    
    # Test Listing
    print("\n--- Testing Listing ---")
    try:
        site_luxuretv.List(site_url)
    ...
    html, _ = utils.get_html_with_cloudflare_retry(site_url, site_url)
    soup = utils.parse_html(html)
    item = soup.select_one('.content, [class*="content"]')
    if item:
        link = item.select_one("a[href]")
        video_url = utils.safe_get_attr(link, "href")
        print(f"Testing playback for: {video_url}")
        try:
            site_luxuretv.Play(video_url, "Test Video")
