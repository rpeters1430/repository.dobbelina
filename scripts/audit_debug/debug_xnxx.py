...
site_xnxx = importlib.import_module("resources.lib.sites.xnxx")

def test_xnxx():
    print("Testing xnxx...")
    site_url = "https://www.xnxx.com/"
    
    # Test Listing
    print("\n--- Testing Listing ---")
    try:
        site_xnxx.List(f"{site_url}todays-selection")
    ...
    html = utils.getHtml(f"{site_url}todays-selection", site_url, headers=hdr)
    soup = utils.parse_html(html)
    item = soup.select_one(".mozaique .thumb-block")
    if item:
        link = item.select_one(".thumb a")
        ...
        try:
            site_xnxx.Playvid(video_url, "Test Video")
