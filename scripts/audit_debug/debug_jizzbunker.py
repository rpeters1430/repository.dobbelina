...
site_jizzbunker = importlib.import_module("resources.lib.sites.jizzbunker")

def test_jizzbunker():
    print("Testing jizzbunker...")
    site_url = "https://jizzbunker.com/"
    
    # Test Listing
    print("\n--- Testing Listing ---")
    try:
        site_jizzbunker.List(f"{site_url}straight/trending")
    ...
    html = utils.getHtml(f"{site_url}straight/trending")
    soup = utils.parse_html(html)
    item = soup.select_one("a.video-card")
    if item:
        ...
        try:
            site_jizzbunker.Playvid(video_url, "Test Video")
