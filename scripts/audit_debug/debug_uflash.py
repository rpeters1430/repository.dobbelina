...
site_uflash = importlib.import_module("resources.lib.sites.uflash")

def test_uflash():
    print("Testing uflash...")
    site_url = "https://www.uflash.tv/"
    
    # Test Listing
    print("\n--- Testing Listing ---")
    try:
        site_uflash.List(f"{site_url}")
    ...
    html = utils._getHtml(site_url, headers=headers)
    soup = utils.parse_html(html)
    item = soup.select_one("li a[href*='/video/']")
    if item:
        ...
        try:
            site_uflash.Playvid(video_url, "Test Video")
