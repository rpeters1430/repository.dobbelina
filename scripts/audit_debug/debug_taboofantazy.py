...
site_taboofantazy = importlib.import_module("resources.lib.sites.taboofantazy")

def test_taboofantazy():
    print("Testing taboofantazy...")
    site_url = "https://www.taboofantazy.com/"
    
    # Test Listing
    print("\n--- Testing Listing ---")
    try:
        site_taboofantazy.List(f"{site_url}?filter=latest")
    ...
    item = soup.select_one('article[data-video-id]')
    if item:
        link = item.select_one("a[href]")
        video_url = utils.safe_get_attr(link, "href")
        print(f"Testing playback for: {video_url}")
        try:
            site_taboofantazy.Play(video_url, "Test Video")
