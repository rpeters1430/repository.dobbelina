...
site_hobbyporn = importlib.import_module("resources.lib.sites.hobbyporn")

def test_hobbyporn():
    print("Testing hobbyporn...")
    site_url = "https://hobby.porn/"
    
    # Test Listing
    print("\n--- Testing Listing ---")
    try:
        site_hobbyporn.List(site_url)
    ...
    print("\n--- Testing Categories ---")
    try:
        site_hobbyporn.Cat(f"{site_url}categories/")
    ...
    item = soup.select_one(".item-video, .video-item")
    if item:
        link = item.select_one("a[href]")
        video_url = utils.safe_get_attr(link, "href")
        print(f"Testing playback for: {video_url}")
        try:
            site_hobbyporn.Playvid(video_url, "Test Video")
