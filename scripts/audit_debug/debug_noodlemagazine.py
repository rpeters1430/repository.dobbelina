...
site_noodlemagazine = importlib.import_module("resources.lib.sites.noodlemagazine")

def test_noodlemagazine():
    print("Testing noodlemagazine...")
    site_url = "https://noodlemagazine.com/"
    
    # Test Listing
    print("\n--- Testing Listing ---")
    try:
        site_noodlemagazine.List(f"{site_url}video/?sort=0&hd=0&len=any&p=1")
    ...
    html, _ = utils.get_html_with_cloudflare_retry(f"{site_url}video/?sort=0&hd=0&len=any&p=1", site_url)
    if html:
        soup = utils.parse_html(html)
        item = soup.select_one('a[href*="/video/"]')
        if item:
            ...
            try:
                site_noodlemagazine.Playvid(video_url, "Test Video")
