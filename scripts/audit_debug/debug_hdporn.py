...
site_hdporn = importlib.import_module("resources.lib.sites.hdporn")

def test_hdporn():
    print("Testing hdporn...")
    site_url = "https://www.porn00.org"
    
    # Test Listing
    print("\n--- Testing Listing ---")
    try:
        site_hdporn.List(f"{site_url}/porn-page/1/")
    ...
    print("\n--- Testing Categories ---")
    try:
        site_hdporn.Cat(f"{site_url}/tags/")
    ...
    html = utils.getHtml(f"{site_url}/porn-page/1/")
    ...
    if item:
        ...
        if not video_url.startswith("http"):
             video_url = site_url + video_url
        print(f"Testing playback for: {video_url}")
        try:
            site_hdporn.Playvid(video_url, "Test Video")
