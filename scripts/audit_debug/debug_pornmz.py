...
site_pornmz = importlib.import_module("resources.lib.sites.pornmz")

def test_pornmz():
    print("Testing pornmz...")
    site_url = "https://pornmz.com/"
    
    # Test Listing
    print("\n--- Testing Listing ---")
    try:
        site_pornmz.List(f"{site_url}page/1?filter=latest")
    ...
    html = utils.getHtml(f"{site_url}page/1?filter=latest", site_url)
    soup = utils.parse_html(html)
    item = soup.select_one("article[data-video-id], article")
    if item:
        ...
        try:
            site_pornmz.Play(video_url, "Test Video")
