...
site_sexyporn = importlib.import_module("resources.lib.sites.sexyporn")

def test_sexyporn():
    print("Testing sexyporn...")
    site_url = "https://www.sexyporn.tv/"
    
    # Test Listing
    print("\n--- Testing Listing ---")
    try:
        site_sexyporn.List(f"{site_url}recent/")
    ...
    html = utils.getHtml(f"{site_url}recent/", site_url)
    soup = utils.parse_html(html)
    item = soup.select_one("div[data-liz]")
    if item:
        ...
        try:
            site_sexyporn.Playvid(video_url, "Test Video")
