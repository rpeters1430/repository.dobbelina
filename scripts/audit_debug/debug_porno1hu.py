...
site_porno1hu = importlib.import_module("resources.lib.sites.porno1hu")

def test_porno1hu():
    print("Testing porno1hu...")
    site_url = "https://porno1.hu/"
    
    # Test Listing
    print("\n--- Testing Listing ---")
    try:
        site_porno1hu.List(f"{site_url}friss-porno/?mode=async&function=get_block&block_id=list_videos_latest_videos_list&sort_by=post_date&from=1")
    ...
    html = utils.getHtml(f"{site_url}friss-porno/?mode=async&function=get_block&block_id=list_videos_latest_videos_list&sort_by=post_date&from=1", site_url)
    ...
    if item:
        ...
        try:
            site_porno1hu.Playvid(video_url, "Test Video")
