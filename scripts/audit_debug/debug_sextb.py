...
site_sextb = importlib.import_module("resources.lib.sites.sextb")

def test_sextb():
    print("Testing sextb...")
    site_url = "https://sextb.net/"
    
    # Test Listing
    print("\n--- Testing Listing ---")
    try:
        site_sextb.List(f"{site_url}uncensored/pg-1")
    ...
    html = utils.getHtml(f"{site_url}uncensored/pg-1", site_url)
    soup = utils.parse_html(html)
    item = soup.select_one(".tray-item, article")
    if item:
        ...
        try:
            site_sextb.Playvid(video_url, "Test Video")
