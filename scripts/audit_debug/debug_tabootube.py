...
site_tabootube = importlib.import_module("resources.lib.sites.tabootube")

def test_tabootube():
    print("Testing tabootube...")
    site_url = "https://www.tabootube.xxx/"
    ajax_url = f"{site_url}?mode=async&function=get_block&block_id=list_videos_most_recent_videos&sort_by=post_date&from=1"
    
    # Test Listing
    print("\n--- Testing Listing ---")
    try:
        site_tabootube.List(ajax_url, 1)
    ...
    html = utils.getHtml(ajax_url, site_url)
    ...
    if item:
        ...
        try:
            site_tabootube.Playvid(video_url, "Test Video")
