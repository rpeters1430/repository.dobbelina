...
site_supjav = importlib.import_module("resources.lib.sites.supjav")

def test_supjav():
    print("Testing supjav...")
    site_url = "https://supjav.com/"
    
    # Test Listing
    print("\n--- Testing Listing ---")
    try:
        site_supjav.List(f"{site_url}popular?sort=week")
    ...
    html = utils.getHtml(f"{site_url}popular?sort=week", site_url)
    soup = utils.parse_html(html)
    item = soup.select_one('.post, [class*="post"]')
    if item:
        link = item.select_one("a[href]")
        video_url = utils.safe_get_attr(link, "href")
        print(f"Testing playback for: {video_url}")
        try:
            site_supjav.Playvid(video_url, "Test Video")
