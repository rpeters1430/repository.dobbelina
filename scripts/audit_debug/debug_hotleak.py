...
site_hotleak = importlib.import_module("resources.lib.sites.hotleak")

def test_hotleak():
    print("Testing hotleak...")
    site_url = "https://hotleak.vip/"
    
    # Test Listing
    print("\n--- Testing Listing ---")
    try:
        site_hotleak.List(f"{site_url}videos")
    ...
    html = utils.getHtml(f"{site_url}videos")
    soup = utils.parse_html(html)
    item = soup.select_one("article.movie-item")
    if item:
        ...
        try:
            site_hotleak.Playvid(video_url, "Test Video")
