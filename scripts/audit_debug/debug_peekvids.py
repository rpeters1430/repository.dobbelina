...
site_peekvids = importlib.import_module("resources.lib.sites.peekvids")

def test_peekvids():
    print("Testing peekvids...")
    site_url = "https://www.peekvids.com/"
    
    # Test Listing
    print("\n--- Testing Listing ---")
    try:
        site_peekvids.List(f"{site_url}Trending-Porn")
    ...
    html = utils.getHtml(f"{site_url}Trending-Porn", site_url)
    soup = utils.parse_html(html)
    item = soup.select_one(".card.itemVideo")
    if item:
        ...
        try:
            site_peekvids.Play(video_url, "Test Video")
