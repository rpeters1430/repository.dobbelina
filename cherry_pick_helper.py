...
def cherry_pick_commit(commit_sha: str, commit_msg: str) -> bool:
    ...
    if code == 0:
        ...
        if "conflict" in stderr.lower() or "conflict" in stdout.lower():
            ...
            print("\nOptions:")
            print(
                "  1. Resolve conflicts manually, then run: git add <files> && git cherry-pick --continue"
            )
...
def apply_phase(phase_key: str, interactive: bool = True):
    ...
    for sha, msg in phase["commits"]:
        if interactive:
            print(f"\n📋 Next: {sha} - {msg}")
            response = input(
                "Apply this commit? (y/n/d=details/s=skip phase/q=quit): "
            ).lower()
        ...
        if cherry_pick_commit(sha, msg):
            ...
            if interactive:
                response = input(
                    "\nConflict occurred. Continue with next commit? (y/n): "
                ).lower()
                if response != "y":
