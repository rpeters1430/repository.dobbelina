...
def _handle_custom_site_failure(module_name, error, dependency=None):
    ...
    utils.kodilog(
        'Custom site import failed [{0}] {1}: {2}'.format(error_kind, module_name, error),
        xbmc.LOGERROR,
    )
    ...
    if dependency:
        message_lines.append('Missing dependency: [COLOR red]{0}[/COLOR]'.format(dependency))
    else:
        message_lines.append('Import error detected inside the site module.')
    message_lines.append('The site has been disabled; reinstall or fix the issue to re-enable it.')
...
def INDEX():
    ...
    if addon_get_setting('custom_sites') == 'true':
        url_dispatcher.add_dir('[COLOR white]Custom site health[/COLOR]', '', 'custom_sites_health',
                               basics.cum_image('cum-sites.png'), '', list_avail=False)
...
def change():
    ...
    if addon_get_setting('changelog_seen_version') == version or not os.path.isfile(basics.changelog):
        return
