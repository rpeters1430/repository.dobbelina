...
orders = {
    ...
    "site & date": "MODE, ROWID DESC",
    "site & name": "MODE, DOMAIN, NAME COLLATE NOCASE",
    "site & date, in folders": "MODE, ROWID DESC",
    "site & name, in folders": "MODE, NAME COLLATE NOCASE",
}
...
def List(url=1):
    ...
    if page == 1:
        basics.addDir(
            "[COLOR red]Refresh images[/COLOR]",
            "",
            "favorites.Refresh_images",
            "",
            Folder=False,
        )
        basics.addDir(
    ...
    try:
        if "folders" in favorder:
            if basics.addon.getSetting("custom_sites") == "true":
                sql = (
                    "SELECT domain, f.mode, COUNT(*) count FROM __favorites f WHERE substr(f.mode, 1, instr(f.mode, '.') - 1) NOT IN (SELECT 'custom_' || cs.name || '_by_' || cs.author FROM custom_sites cs "
            ...
            if len(folders) > offset + items_per_page:
                basics.addDir(
                    "[COLOR orange]Next Page ({}/{})[/COLOR]".format(page + 1, pages),
                    str(page + 1),
                    "favorites.List",
                    "",
                )
            ...
            if basics.addon.getSetting("custom_sites") == "true":
                sql = """SELECT domain, name, url, mode, image, duration, quality
                        ...
                        ORDER BY {} LIMIT {} OFFSET {}""".format(
                    orders[favorder], items_per_page, offset
                )
                ...
                sql = """SELECT domain, name, url, mode, image, duration, quality FROM __favorites f
                        WHERE mode NOT LIKE 'custom_%' ORDER BY {} LIMIT {} OFFSET {}""".format(
                    orders[favorder], items_per_page, offset
                )
                count_sql = (
                    "SELECT COUNT(*) FROM __favorites f WHERE mode NOT LIKE 'custom_%'"
                )
...
def FavListSite(url, page=1):
    ...
    try:
        ...
        c.execute(
            "SELECT COUNT(*) FROM __favorites WHERE mode = ? and domain = ?",
            (mode, domain),
        )
        total_items = c.fetchone()[0]
        pages = (total_items - 1) // items_per_page + 1
        ...
        c.execute(
            "SELECT name, url, mode, image, duration, quality FROM __favorites WHERE mode = ? and domain = ? ORDER BY {} LIMIT ? OFFSET ?".format(
                orders[favorder]
            ),
            (mode, domain, items_per_page, offset),
        )
...
def update_favorite(mode, name, url, img, duration, quality):
    ...
    c = conn.cursor()
    c.execute(
        "UPDATE favorites set name = ?, image = ?, mode = ?, duration = ?, quality = ? where url = ?",
...
def move_fav_to_top(url):
    ...
    c = conn.cursor()
    c.execute(
        "UPDATE favorites SET rowid = (SELECT MAX(rowid) FROM favorites) + 1 WHERE url = ?",
        (url,),
    )
    conn.commit()
    conn.close()
...
def move_fav_to_bottom(url):
    ...
    c = conn.cursor()
    c.execute(
        "UPDATE favorites SET rowid = (SELECT MIN(rowid) FROM favorites) - 1 WHERE url = ?",
        (url,),
    )
    conn.commit()
    conn.close()
...
def clear_fav():
    if not utils.dialog.yesno(
        "Warning",
        "This will delete all your favorites[CR]Continue?",
        nolabel="No",
        yeslabel="Yes",
    ):
        return
...
def backup_fav():
    ...
    backup_content = {
        "meta": {"type": "cumination-favorites", "version": 1, "datetime": time},
        "data": favorites,
    }
...
def restore_fav():
    ...
    for favorite in favorites:
        if select_favorite(favorite["url"]):
            ...
            favorite["mode"].startswith("custom_")
            and favorite["mode"].split(".")[0] not in custom_sites
        ):
            utils.kodilog(
                "{} is not installed, skipping".format(favorite["mode"].split(".")[0])
            )
            skipped += 1
        else:
...
def install_custom_sites_from_folder():
    ...
    if not utils.dialog.yesno(
        "WARNING", "All custom sites from the list will be installed. Proceed?"
    ):
        progress.close()
        return
    successful = []
...
def process_custom_site_zip(path):
    ...
    if already_installed:
        ...
        utils.textBox(
            ...
            "Custom site is already installed[CR]Title: {}[CR]Version: {}[CR][CR]"
            "New title: {}[CR]New version: {}[CR]".format(
                old_title, old_version, title, version
            ),
        )
        if not utils.dialog.yesno(
            "Site already installed",
            "Replace version {} with {}?".format(old_version, version),
        ):
            basics.clean_temp()
            return False
        keep_favorites = utils.dialog.yesno(
            "Site already installed",
    ...
    new_image = (
        "{}_{}_img.{}".format(author, name, original_image.split(".")[-1])
        if original_image
        else None
    )
    ...
    add_custom_site(
        ...
        url,
        new_image,
        new_about.split(".")[0] if new_about else None,
        version,
        new_module.split(".")[0],
    )
...
def enable_custom_site():
    ...
    c = conn.cursor()
    c.execute(
        "UPDATE custom_sites SET enabled = 1 WHERE author = ? and name = ?",
        (author, name),
    )
    conn.commit()
    conn.close()
...
def disable_all_custom_sites():
    ...
    if not utils.dialog.yesno(
        "WARNING", "All custom sites will be disabled.[CR]Continue?"
    ):
        return
    conn = sqlite3.connect(favoritesdb)
    conn.text_factory = str
...
def select_custom_sites_attributes(author_and_name, *args):
    ...
    if author_and_name:
        c.execute(
            "SELECT {} FROM custom_sites where author = ? and name = ?".format(
                ", ".join(args)
            ),
...
def delete_custom_site(author, name, keep_favorites=False):
    site_elements = select_custom_sites_attributes(
        (author, name), "module_file", "image", "about", "title"
    )[0]
    removable = (
        site_elements[0] + ".py",
        site_elements[0] + ".pyo",
        site_elements[1],
        site_elements[2] + ".txt" if site_elements[2] else None,
    )
    for r in removable:
        if r:
            try:
...
def get_custom_data(author, name):
    ...
    c = conn.cursor()
    c.execute(
        "SELECT title, image, about, url FROM custom_sites WHERE author = ? AND name = ?",
        (author, name),
    )
    row = c.fetchone()
    conn.close()
...
def get_custom_listitems():
    ...
    c = conn.cursor()
    c.execute(
        "SELECT name, count(*) count FROM custom_listitems WHERE list_id in (SELECT 'main' union SELECT ROWID from custom_lists) group by name"
    )
    rows = c.fetchall()
    conn.close()
    return rows
...
def load_custom_list(url):
    ...
    if basics.addon.getSetting("custom_sites") == "true":
        c.execute(
            "select cli.rowid, cli.name, cli.url, cli.mode, cli.image from custom_listitems cli "
            + "LEFT JOIN custom_sites cs on 'custom_' || cs.name || '_by_' ||"
            + " cs.author = substr(cli.mode, 1, instr(cli.mode, '.') - 1)"
            + " WHERE ifnull(cs.enabled, 1) = 1 and cli.list_id = ?",
            (url,),
        )
    else:
        c.execute(
            ...
            + " LEFT JOIN custom_sites cs on 'custom_' || cs.name || '_by_' ||"
            + " cs.author = substr(cli.mode, 1, instr(cli.mode, '.') - 1)"
            + " WHERE cs.name IS NULL and cli.list_id = ?",
            (url,),
        )
...
def add_listitem(favmode, name, url, img):
    ...
    if selected_id:
        selected_list = next(
            key for key, value in custom_lists.items() if value == selected_id
        )
        c.execute(
            "SELECT count(*) FROM custom_listitems WHERE list_id=? and url=?",
            (selected_id, url),
        )
        if c.fetchone()[0] > 0:
            if utils.dialog.yesno(
                "Item already in list",
                "This item is already in the list [COLOR hotpink]{}[/COLOR][CR]Update the name and image?".format(
                    selected_list
                ),
            ):
                c.execute(
                    "UPDATE custom_listitems set name=?, mode=?, image=? WHERE url=? and list_id=?",
                    (name, favmode, img, url, selected_id),
                )
        else:
            c.execute(
                "INSERT INTO custom_listitems VALUES (?,?,?,?,?)",
                (name, url, favmode, img, selected_id),
            )
        conn.commit()
    conn.close()
    xbmc.executebuiltin("Container.Refresh")
...
def move_listitem(listitem_id):
    ...
    if selected_id:
        ...
        c = conn.cursor()
        c.execute(
            "UPDATE custom_listitems set list_id = ? WHERE rowid = ?",
            (
                selected_id,
                listitem_id,
            ),
...
def edit_list(rowid):
    ...
    c = conn.cursor()
    c.execute(
        "UPDATE custom_lists set name = ? WHERE rowid = ?",
        (
            name,
            int(rowid),
        ),
