...
def Main():
    ...
    site.add_dir(
        "[COLOR hotpink]Country: [/COLOR] [COLOR orange]{}[/COLOR]".format(country),
        site.url,
        "Country",
        site.img_cat,
    )
    site.add_dir(
        "[COLOR hotpink]Category: [/COLOR] [COLOR orange]{}[/COLOR]".format(category),
        site.url,
        "Category",
        site.img_cat,
    )
    ...
    site.add_dir(
        ...
        site.url + "pornstars-index/{}{}from/worldwide".format(cat, country),
        "Pornstars",
        site.img_cat,
    )
...
def List(url):
    ...
    for item in video_items:
        ...
        img = utils.safe_get_attr(
            img_tag,
            "data-src",
            ["data-mediumthumb", "data-thumb-url", "data-srcset", "src"],
        )
        if not img:
            continue
        if "THUMBNUM" in img:
        ...
        if meta:
            ...
            meta_parts = [
                part.strip() for part in meta_text.split(" - ") if part.strip()
            ]
            if duration:
    ...
    if pagination:
        ...
        if next_link:
            ...
            if npage:
                ...
                site.add_dir(
                    "Next Page ({}{})".format(np, lp), npage, "List", site.img_next
                )
...
def Categories(url):
    ...
    match = re.compile(
        r'href="([^"]+)">([^<]+)<[^"]+class="dyn', re.DOTALL | re.IGNORECASE
    ).findall(cathtml)
...
def PSList(url):
    ...
    for video in jdata["videos"]:
        ...
        namef = utils.cleantext(
            video["tf"] if utils.PY3 else video["tf"].encode("utf-8")
        )
        duration = video["d"]
...
def Pornstars(url):
    ...
    npage = re.compile(
        r'href="([^"]+)" class="no-page next-page', re.DOTALL | re.IGNORECASE
    ).findall(cathtml)
    if npage:
...
def Playvid(url, name, download=None):
    ...
    match = re.search(r"html5player\.setVideoHLS\('([^']+)", videopage)
    if not match:
...
def get_setting(x):
    ...
    if x == "sortby":
        ret = (
            utils.addon.getSetting("xvideossortby")
            if utils.addon.getSetting("xvideossortby")
            else "relevance"
        )
    if x == "date":
        ret = (
            utils.addon.getSetting("xvideosdate")
            if utils.addon.getSetting("xvideosdate")
            else "anytime"
        )
    if x == "length":
        ret = (
            utils.addon.getSetting("xvideoslen")
            if utils.addon.getSetting("xvideoslen")
            else "all"
        )
    if x == "quality":
        ret = (
            utils.addon.getSetting("xvideosqual")
            if utils.addon.getSetting("xvideosqual")
            else "all"
        )
    if x == "country":
        ret = (
            utils.addon.getSetting("xvideoscountry")
            if utils.addon.getSetting("xvideoscountry")
            else "USA"
        )
    if x == "category":
        ret = (
            utils.addon.getSetting("xvideoscategory")
            if utils.addon.getSetting("xvideoscategory")
            else "Straight"
        )
    return ret
...
def update_url(url):
    ...
    if "k=" in url:
        if (sortby == "relevance" and "sort=" in url) or (
            sortby != "relevance" and "sort=" + sortby not in url
        ):
            url = re.sub(r"[\?&]sort=[^\?&]+", "", url)
            url = re.sub(r"[\?&]p=[^\?&]+", "", url)
            url += "&sort=" + sortby if sortby != "relevance" else ""
        if (date == "anytime" and "datef=" in url) or (
            date != "anytime" and "datef=" + date not in url
        ):
            url = re.sub(r"[\?&]datef=[^\?&]+", "", url)
            url = re.sub(r"[\?&]p=[^\?&]+", "", url)
            url += "&datef=" + date if date != "anytime" else ""
        if (length == "all" and "durf=" in url) or (
            length != "all" and "durf=" + length not in url
        ):
            url = re.sub(r"[\?&]durf=[^\?&]+", "", url)
            url = re.sub(r"[\?&]p=[^\?&]+", "", url)
            url += "&durf=" + length if length != "all" else ""
        if (quality == "all" and "quality=" in url) or (
            quality != "all" and "quality=" + quality not in url
        ):
    ...
    if type:
        if (quality == "all" and "/q:" in url) or (
            quality != "all" and "/q:" + quality not in url
        ):
            url = re.sub(r"/q:[^/]+", "", url)
            url = re.sub(r"/\d+$", "", url)
            if quality != "all":
                url = url.replace(site.url + type, site.url + type + "/q:" + quality)
        if (length == "all" and "/d:" in url) or (
            length != "all" and "/d:" + length not in url
        ):
            url = re.sub(r"/d:[^/]+", "", url)
            url = re.sub(r"/\d+$", "", url)
            if length != "all":
                url = url.replace(site.url + type, site.url + type + "/d:" + length)
        if (date == "anytime" and "/m:" in url) or (
            date != "anytine" and "/m:" + date not in url
        ):
            url = re.sub(r"/m:[^/]+", "", url)
            if type != "tags":
                url = re.sub(r"/\d+$", "", url)
        ...
        if (sortby == "relevance" and "/s:" in url) or (
            sortby != "relevance" and "/s:" + sortby not in url
        ):
            url = re.sub(r"/s:[^/]+", "", url)
            url = re.sub(r"/\d+$", "", url)
            if sortby != "relevance":
