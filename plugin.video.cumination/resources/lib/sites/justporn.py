...
VIDEO_LIST_SPEC = SoupSiteSpec(
    selectors={
        "base_url": site.url,
        "items": ["div.content", "div.video-item", "article.video-item"],
...
def List(url):
    ...
    if match:
        ...
        cm_page = (
            utils.addon_sys
            + "?mode=justporn.GotoPage&list_mode=justporn.List&url="
...
def Lookupinfo(url):
    lookup_list = [
        (
            "Cat",
            r'href="(/category/[^"]+)" style="cursor: pointer">\s*([^<]+)\s*</a',
            "",
        ),
        (
            "Tag",
            r'href="(/tag/[^"]+)" style="cursor: pointer">\s*<i>#</i>([^<]+)\s*</a',
            "",
        ),
    ]
