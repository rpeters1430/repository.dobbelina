...
VIDEO_LIST_SPEC = SoupSiteSpec(
    selectors={
        "items": "div.thumb.item",
        "url": {"selector": "a", "attr": "href"},
        "title": {
            "selector": "div.title",
            "fallback_selectors": ["a[title]"],
            "text": True,
            "clean": True,
        },
        "thumbnail": {
        ...
        "duration": {"selector": "div.time", "text": True},
        "pagination": {
            "selector": ".navigation a, .mobnavigation a",
            "text_matches": ["Next", ">"],
            "attr": "href",
            "mode": "List",
        },
...
def Playvid(url, name, download=None):
    ...
    # Check for direct sources in player markup
    match = re.compile(
        r'<source src="([^"]+)" title="([^"]+)"', re.IGNORECASE | re.DOTALL
    ).findall(html)
    if match:
        sources = {m[1]: urllib_parse.urljoin(site.url, m[0].replace("&amp;", "&")) for m in match}
