...
def List(url):
    ...
    for item in video_items:
        ...
        title_tag = item.select_one(".thumb__title span") or item.select_one(
            ".thumb__title"
        )
        name = (
            utils.safe_get_text(title_tag)
            or utils.safe_get_attr(img_tag, "alt")
            or utils.safe_get_attr(top_link, "title")
        )
        ...
        img = utils.safe_get_attr(
            img_tag, "data-src", ["data-original", "data-lazy", "src", "srcset"]
        )
        if not img:
            continue
        if "," in img:
        ...
        qual_text = (
            utils.safe_get_text(item.select_one(".thumb__bage, .thumb__badge")) or ""
        ).lower()
        if qual_text == "720p":
            hd_text = "HD "
        ...
        site.add_download_link(
            name, videopage, "Play", img, name, duration=duration, quality=hd_text
        )
    ...
    if pagination:
        ...
        if next_link:
            ...
            if next_href:
                ...
                if next_hint:
                    label_hint = (
                        "{}/{}".format(next_hint, last_hint) if last_hint else next_hint
                    )
...
def Cat(url):
    ...
    for anchor in soup.select("div.letter-block__item a.letter-block__link"):
        ...
        name = utils.safe_get_text(anchor.select_one("span")) or utils.safe_get_text(
            anchor
        )
