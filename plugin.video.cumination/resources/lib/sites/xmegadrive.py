...
site = AdultSite('xmegadrive', '[COLOR hotpink]xMegaDrive[/COLOR]', 'https://www.xmegadrive.com/', 'xmegadrive.png', 'xmegadrive')
...
def _parse_flashvars(html):
    ...
    if not block:
        block = re.search(r'flashvars\s*=\s*\{([^}]+)\}', html, re.IGNORECASE | re.DOTALL)
    if not block:
        return {}
...
def List(url):
    ...
    for videourl, title, img, duration in items:
        ...
        site.add_download_link(display_name, videourl, 'Playvid', img, title, duration=duration)
...
def Categories(url):
    ...
    if nurl:
        if nurl.startswith('/'):
            nurl = urllib_parse.urljoin(site.url, nurl)
        site.add_dir('[COLOR hotpink]Next Page...[/COLOR]', nurl, 'Categories', site.img_next)
