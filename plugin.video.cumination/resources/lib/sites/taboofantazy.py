...
site = AdultSite(
    "taboofantazy",
    "[COLOR hotpink]TabooFantazy[/COLOR]",
    "https://www.taboofantazy.com/",
    "taboofantazy.png",
    "taboofantazy",
)
...
def List(url):
    ...
    for article in articles:
        ...
        hd = (
            "HD"
            if hd_badge and "HD" in utils.safe_get_text(hd_badge, default="")
            else ""
        )
        ...
        site.add_download_link(
            name, videourl, "Play", img, name, contextm=contextmenu, quality=hd
        )
    ...
    if current_page:
        ...
        if next_sibling:
            ...
            if next_url and "/categories/" in next_url:
                site.add_dir(
                    "[COLOR hotpink]Next Page[/COLOR]", next_url, "Cat", site.img_next
                )
...
def Play(url, name, download=None):
    vp = utils.VideoPlayer(
        name, download=download, regex=r'itemprop="embedURL"\s*content="([^"]+)"'
    )
