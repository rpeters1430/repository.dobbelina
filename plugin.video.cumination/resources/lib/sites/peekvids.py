...
def _add_pagination(soup, mode):
    ...
    site.add_dir(
        "[COLOR hotpink]Next Page...[/COLOR] ({0})".format(label),
        next_url,
        mode,
        site.img_next,
    )
...
def List(url):
    ...
    for card in cards:
        ...
        img = utils.safe_get_attr(
            img_tag, "data-src", ["data-original", "data-thumbnail", "src"]
        )
        if img and img.startswith("//"):
            img = "https:" + img

        duration_tag = card.select_one(
            ".info, .meta__duration, .duration, .card__duration, .video-duration, .time"
        )
        duration = utils.safe_get_text(duration_tag)

        hd = ""
        quality_tag = card.select_one(
            ".quality, .hd, .label-hd, .badge, .video-quality, .is_hd, .tag"
        )
        if quality_tag:
        ...
        site.add_download_link(
            name, videopage, "Play", img, name, duration=duration, quality=hd
        )
...
def Cat(url):
    ...
    category_items = soup.select(
        ".categories-list li, .categories li, ul.categories li"
    )
...
def Channels(url):
    ...
    for card in channel_cards:
        ...
        vids_tag = card.select_one(
            '.videos, .video-count, .channel-videos, [class*="videos"]'
        )
