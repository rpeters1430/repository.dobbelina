...
def Main():
    site.add_dir(
        "[COLOR hotpink]Models[/COLOR]", site.url + "actor/", "ActorABC", site.img_cat
    )
    site.add_dir("[COLOR hotpink]Tags[/COLOR]", site.url + "tag/", "Tags", site.img_cat)
    site.add_dir(
        "[COLOR hotpink]Search[/COLOR]",
        site.url + "video/search/",
        "Search",
        site.img_search,
    )
...
def Playvid(url, name, download=None):
    ...
    r = re.compile(r"src=\\u0022([^ ]+)\\u0022", re.DOTALL | re.IGNORECASE).search(
        videopage
    )
    if r:
        ...
        r = re.compile(r'<iframe.+?src="([^"]+)', re.DOTALL | re.IGNORECASE).search(
            videopage
        )
        if r:
            vp.progress.update(75, "[CR]Video found[CR]")
