...
def Categories(url):
    ...
    for item in soup.select("a.cat_item"):
        ...
        name = utils.cleantext(
            utils.safe_get_text(item.select_one(".cat_ttl"), default="")
        )
        ...
        site.add_dir(
            name, site.url[:-1] + catpage, "List", site.url[:-1] + image, desc=desc
        )
    utils.eod()
...
def Series(url, section=None):
    ...
    for item in soup.select("a.vc_item"):
        ...
        name = utils.cleantext(
            utils.safe_get_text(item.select_one(".vcat_title"), default="")
        )
        img = utils.safe_get_attr(
            item.select_one(".vcat_poster img"), "data-src", ["src"]
        )
    ...
    next_page_link = soup.find(
        "a", class_="page-link", string=lambda t: t and "Next" in t
    )
    if next_page_link:
        page_num_match = re.search(
            r"page=(\d+)", utils.safe_get_attr(next_page_link, "href", default="")
        )
        page_label = f" ({page_num_match.group(1)})" if page_num_match else ""
        site.add_dir(
            f"[COLOR hotpink]Next Page[/COLOR]{page_label}",
            site.url[:-1] + utils.safe_get_attr(next_page_link, "href", default=""),
            "Series",
            site.img_next,
        )
