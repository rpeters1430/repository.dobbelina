...
def List(url):
    ...
    container = soup.select_one("ul.thumb-list") or soup.find(
        "ul", class_=lambda c: c and "thumb-list" in c
    )
    if not container:
        utils.eod()
        return
    ...
    for item in container.select("li"):
        ...
        img = utils.safe_get_attr(
            img_tag, "src", ["data-src", "data-original", "data-lazy"]
        )
    ...
    if next_link:
        site.add_dir(
            "Next Page",
            urljoin(site.url, utils.safe_get_attr(next_link, "href")),
            "List",
            site.img_next,
        )
...
def Stars(url):
    ...
    if container:
        for item in container.select("li"):
            ...
            vidcount = utils.safe_get_text(
                item.select_one(".vids, .videos, .count"), default=""
            ).strip()
            if vidcount:
                name = "{0}[COLOR orange] [COLOR deeppink][I]({1})[/I][/COLOR]".format(
                    name, vidcount
                )
    ...
    if nextp:
        site.add_dir(
            "Next Page",
            urljoin(site.url, utils.safe_get_attr(nextp, "href")),
            "Stars",
            site.img_next,
        )
    utils.eod()
...
def Playvid(url, name, download=None):
    ...
    sources = [
        utils.safe_get_attr(tag, "src")
        for tag in click_soup.find_all(["source", "a", "video"], src=True)
    ]
