...
site = AdultSite(
    "hentai-moon",
    "[COLOR hotpink]Hentai Moon[/COLOR]",
...
def Main():
    site.add_dir(
        ...
        site.url
        + "categories/?mode=async&function=get_block&block_id=list_categories_categories_list&sort_by=title",
        "Categories",
        site.img_cat,
    )
    site.add_dir(
        ...
        site.url
        + "series/?mode=async&function=get_block&block_id=list_dvds_channels_list&sort_by=title&from=1",
        "Series",
        site.img_cat,
    )
    site.add_dir(
    ...
    site.add_dir(
        ...
        site.url + "top-rated/" + ajaxtop,
        "List",
        site.img_cat,
    )
    site.add_dir(
        "[COLOR hotpink]Most Viewed[/COLOR]",
        site.url + "most-popular/" + ajaxmost,
        "List",
        site.img_cat,
    )
    site.add_dir(
...
def Categories(url):
    ...
    for link in soup.select("a.item[href][title], div.item a[href][title]"):
        ...
        label = (
            name
            if not videos
            else "{0} [COLOR deeppink]{1}[/COLOR]".format(name, videos)
        )
...
def Series(url):
    ...
    if not soup:
        ...
        return
    for link in soup.select("a.item[href][title], div.item a[href][title]"):
        seriepage = urllib_parse.urljoin(
            site.url, utils.safe_get_attr(link, "href", default="")
        )
        name = utils.cleantext(utils.safe_get_attr(link, "title", default=""))
        ...
        img = utils.safe_get_attr(
            link.select_one("img"), "data-original", fallback_attrs=["src"], default=""
        )
...
def Search(url, keyword=None):
    if not keyword:
        ...
        url = "{0}{1}/?mode=async&function=get_block&block_id=list_videos_videos_list_search_result&q={2}&cat_ids=&sort_by=&from_videos=1".format(
            url, title, searchq
        )
        List(url)
...
def _lookup_context_menu(video_url):
    contexturl = (
        utils.addon_sys
        + "?mode="
        + str("hentai-moon.Lookupinfo")
        + "&url="
        + urllib_parse.quote_plus(video_url)
    )
...
def _add_next_page(soup, current_url, mode):
    if not soup:
        return
    next_link = soup.find(
        lambda tag: tag.name == "a" and tag.string and "next" in tag.string.lower()
    )
    next_url = ""
    if next_link:
        ...
        if not next_url:
            ...
            if match:
                ...
                next_url = "{0}?{1}".format(
                    base, urllib_parse.urlencode(query, doseq=True)
                )
    if next_url:
