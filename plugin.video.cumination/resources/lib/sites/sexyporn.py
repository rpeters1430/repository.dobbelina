...
site = AdultSite('sexyporn', '[COLOR hotpink]SexyPorn[/COLOR]', 'https://www.sexyporn.xxx/', 'sexyporn.png', 'sexyporn')


@site.register(default_mode=True)
def Main():
    site.add_dir('[COLOR hotpink]Categories[/COLOR]', urllib_parse.urljoin(site.url, 'categories'), 'Categories', site.img_cat)
    site.add_dir('[COLOR hotpink]Search[/COLOR]', urllib_parse.urljoin(site.url, 'search'), 'Search', site.img_search)
    List(site.url)
    utils.eod()
...
def List(url):
    ...
    if pagination:
        next_url = urllib_parse.urljoin(url, utils.safe_get_attr(pagination, 'href', default=''))
