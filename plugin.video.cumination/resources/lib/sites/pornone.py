...
def List(url):
    ...
    if next_link:
        ...
        utils.kodilog(
            'pornone: Found pagination via link[rel="next"]: {}'.format(next_url)
        )
    ...
    if not next_url:
        ...
        if pagination:
            next_button = pagination.select_one(
                'a[class*="next"], a:-soup-contains("Next"), a:-soup-contains(">")'
            )
            if next_button:
                next_url = utils.safe_get_attr(next_button, "href")
                utils.kodilog(
                    "pornone: Found pagination via next button: {}".format(next_url)
                )
    ...
    if not next_url:
        ...
        if current_page_match:
            ...
            utils.kodilog(
                "pornone: Constructed next page URL from current page {}: {}".format(
    ...
    if next_url:
        ...
        if next_url != url:
            site.add_dir(
                "[COLOR hotpink]Next Page...[/COLOR] ({0})".format(next_page_num),
                next_url,
                "List",
                site.img_next,
            )
            ...
            utils.kodilog(
                "pornone: Next page URL same as current URL - pagination loop detected and prevented"
            )
