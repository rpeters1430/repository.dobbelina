...
def _extract_best_source(html):
    ...
    utils.kodilog(
        "RedTube: Regex match result: {}".format("FOUND" if match else "NOT FOUND")
    )
    ...
    if not match and "mediaDefinitions" in html:
        ...
        utils.kodilog(
            "RedTube: Context: {}".format(html[max(0, idx - 100) : idx + 200])
        )
    elif not match:
        ...
        if "<title>" in html[:500]:
            ...
            if title_match:
                utils.kodilog(
                    "RedTube: Page title: {}".format(title_match.group(1)[:100])
                )
    ...
    if match:
        ...
        try:
            ...
            utils.kodilog(
                "RedTube: Parsed {} items from mediaDefinitions".format(len(items))
            )
            for item in items:
                        ...
                        if api_response:
                            ...
                            for quality_item in api_data:
                                ...
                                if video_url and quality:
                                    ...
                                    utils.kodilog(
                                        "RedTube: Found {}p: {}".format(
                                            quality, video_url[:100]
                                        )
                                    )
                    except Exception as e:
    ...
    utils.kodilog(
        "RedTube: Selected best quality: {}p - {}".format(
