...
def Playvid(url, name, download=None):
    ...
    # Try sniffing if direct match fails
    try:
        from resources.lib.playwright_helper import sniff_video_url
        vp.progress.update(40, "[CR]Sniffing with Playwright...[CR]")
        
        video_url = sniff_video_url(url, play_selectors=["video", "#kt_player"])
        if video_url:
            utils.kodilog("pornxpert: Playwright found stream: {}".format(video_url[:100]))
            vp.play_from_direct_link(video_url + "|Referer=" + site.url + "&User-Agent=" + utils.USER_AGENT)
            return
    except (ImportError, Exception) as e:
        utils.kodilog("pornxpert: Playwright sniffer failed: {}".format(e))
