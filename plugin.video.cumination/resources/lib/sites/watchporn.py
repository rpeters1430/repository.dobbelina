...
def List(url):
    ...
    for item in video_items:
        try:
            ...
            duration_tag = item.select_one(
                ".duration, .item__time, .time, .item__meta .meta__time"
            )
            duration = utils.safe_get_text(duration_tag)

            quality_tag = item.select_one(
                ".is-hd, .is-4k, .is-1080p, .is-720p, .is-480p, .quality, .item__quality"
            )
    ...
    for candidate in soup.select("a.next, a[data-block-id]"):
        ...
        if "next" in text or "page-next" in classes:
            ...
            break
    if next_link:
        current_span = soup.select_one(
            ".page-current span, span.page-current, li.active span"
        )
        try:
        ...
        if block_id and params:
            ...
            next_page_url = re.sub(
                r"&from([^=]*)=\d+", rf"&from\1={next_page_num}", nurl
            )
        else:
        ...
        if next_page_url:
            cm_page_url = (
                f"{utils.addon_sys}?mode=watchporn.GotoPage&url="
...
def Categories(url):
    ...
    for anchor in soup.select("a.item[href], a.categories-list__item[href]"):
        ...
        img = None
        if img_tag:
            thumb_url = utils.safe_get_attr(
                img_tag, "src", ["data-original", "data-src", "data-lazy"]
            )
            if thumb_url:
