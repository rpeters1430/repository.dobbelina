...
site = AdultSite(
    "theyarehuge",
    "[COLOR hotpink]They Are Huge[/COLOR]",
...
def Main():
    site.add_dir(
        ...
        site.url
        + "popular.porn-video/?mode=async&function=get_block&block_id=list_videos_common_videos_list&sort_by=video_viewed&from=1",
        "List",
        site.img_cat,
        page=1,
    )
    ...
    List(
        site.url
        + "latest-updates/?mode=async&function=get_block&block_id=list_videos_latest_videos_list&sort_by=post_date&from=1",
        1,
    )
    utils.eod()
...
def Playvid(url, name, download=None):
    ...
    if "720p" in videourl and tahlogged:
        ...
        vp.play_from_direct_link(
            videourl + "|referer=" + url + "&User-Agent=" + utils.USER_AGENT
        )
...
def Search(url, keyword=None):
    ...
    if not keyword:
        ...
        searchUrl = (
            searchUrl
            + title
            + "/?mode=async&function=get_block&block_id=list_videos_videos_list_search_result&sort_by=post_date&from_videos=1"
        )
        List(searchUrl, 1)
...
def Lookupinfo(url):
    ...
    lookup_list = [
        ...
        (
            "Model",
            r'tag-pornstar"\s+href="https://www\.theyarehuge\.com/([^"]+)">([^<]+)</a>',
            "",
        ),
    ]
...
def TAHLogin(logged=True):
    ...
    if not tahlogged or "false" in tahlogged:
        ...
        if tahuser == "":
            tahuser = getinput(
                default=tahuser, heading="Input your They are huge username"
            )
            tahpass = getinput(
                default=tahpass,
                heading="Input your They are Huge password",
                hidden=True,
            )
        ...
        if clear:
            ...
            contexturl = utils.addon_sys + "?mode=theyarehuge.Main"
