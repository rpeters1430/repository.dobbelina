...
def Main():
    site.add_dir(
        ...
        site.url + "php/model_tags.php?vcc=1545329519",
        "Tags",
        "",
    )
    List(
        site.url
        + "php/model_explorer.php?get_contents=1&sort=cam_score&selection=public&page=1"
    )
...
def Tags(url):
    url = (
        site.url
        + "php/model_tags.php?get_tags=1&tag_sort=&word_source=tags&display_style=list&member_mode=0"
    )
    ...
    for item, models in res:
        url = (
            site.url
            + "php/model_tags.php?get_users=1&selected_field={0}&display_style=list".format(
                urllib_parse.quote_plus(item)
            )
            + "&word_source=tags&member_mode=0&page=1&stand_alone=true"
        )
        site.add_dir(
            "{0} [COLOR hotpink]{1}[/COLOR]".format(item, models),
            url,
            "TagsList",
            "",
            "",
...
def myfreecam_start(url):
    ...
    if CAMGIRLSERVER != 0:
        if PHASE == "":
            Url = "https://{0}.myfreecams.com/NxServer/ngrp:mfc_{1}.f4v_mobile/playlist.m3u8?nc=0.5863279394620062".format(
                CAMGIRLSERVER, CAMGIRLCHANID
            )
        else:
            Url = "https://{0}.myfreecams.com/NxServer/ngrp:mfc_a_{1}.f4v_mobile/playlist.m3u8?nc=0.5863279394620062".format(
                CAMGIRLSERVER, CAMGIRLCHANID
            )
