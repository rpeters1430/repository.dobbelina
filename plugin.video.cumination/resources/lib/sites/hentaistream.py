...
def Main():
    site.add_dir(
        ...
        site.url + "search?order=recently-uploaded&page=1",
        "Tags",
        site.img_cat,
    )
    site.add_dir(
        "[COLOR hotpink]Search[/COLOR]",
        site.url + "search?search=",
        "Search",
        site.img_search,
    )
...
def List(url, episodes=True):
    ...
    for item in items:
        ...
        if name and img:
            ...
            fanart_img = img
            cover_img = fanart_img.replace("gallery", "cover").replace(
                "-0-thumbnail", ""
            )
    ...
    if next_link:
        ...
        if pagelookup:
            ...
            url = url.replace(
                "page={0}".format(pagelookup.group(1)), "page={0}".format(np)
            )
...
def Tags(url):
    ...
    for tag_id, name in sorted(tags_list, key=lambda x: x[1]):
        name = utils.cleantext(name)
        tagpage = site.url + "search?order=recently-uploaded&page=1&tags[0]={}".format(
            tag_id
        )
...
def Playvid(url, name, download=None):
    ...
    videojson = utils._postHtml(
        site.url + "player/api", headers=hstreamhdrs, json_data=payload
    )
    videojson = json.loads(videojson)

    qualities = {
        "2160": "/2160/manifest.mpd",
