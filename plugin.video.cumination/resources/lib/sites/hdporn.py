...
def List(url):
    ...
    match = re.compile(
        r'class="item.+?href="([^"]+).+?original="([^"]+)(.+?)le">\s*([^<]+).+?on">([^<]+)',
    ...
    if npage:
        ...
        site.add_dir(
            "Next Page ({0})".format(purl.split("/")[-2]), purl, "List", site.img_next
        )
...
def Cat(url):
    ...
    cathtml = re.compile('class="box"(.+?)"footer', re.DOTALL | re.IGNORECASE).findall(
        caturl
    )
    match = re.compile(
        """<li.*?href=['"]([^'"]+).*?>([^<]+)""", re.DOTALL | re.IGNORECASE
    ).findall(cathtml[0])
