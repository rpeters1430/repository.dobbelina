...
site = AdultSite(
    "seaporn",
    "[COLOR hotpink]SeaPorn[/COLOR] [COLOR red][Debrid only][/COLOR]",
...
def List(url, page=1):
    ...
    while len(videodict) < 40:
        ...
        for videopage, rlsname, posted, img in match:
            ...
            progress.update(
                int(len(videodict.keys()) * 100 / 40),
                "Loading video {}".format(len(videodict.keys()) + 1),
            )
    ...
    for name, videos in videodict.items():
        ...
        contextmenu.append(
            ("[COLOR deeppink]Lookup info[/COLOR]", "RunPlugin(" + contextm_info + ")")
        )
        if len(videos) > 1:
            ...
            contextm_verions = (
                ...
                + "&url="
                + urllib_parse.quote_plus(videolist_json)
            )
            contextmenu.append(
                (
                    "[COLOR deeppink]Other qualities[/COLOR]",
                    "Container.Update(" + contextm_verions + ")",
                )
            )

        site.add_download_link(
            rlsname, videopage, "Playvid", img, plot, contextm=contextmenu
        )
...
def Playvid(url, name, download=None):
    ...
    sources = re.compile(
        '<a href="([^"]+)" class="autohyperlink">https*://([^/]+)',
        re.DOTALL | re.IGNORECASE,
    ).findall(sitehtml)
    links = {}
...
def Categories(url):
    ...
    for catpage, name, videos in match:
        ...
        if any(
            cat in name
            for cat in ["Galleries", "Magazines", "Pictures", "Siterips", "Mobile"]
        ):
            continue
