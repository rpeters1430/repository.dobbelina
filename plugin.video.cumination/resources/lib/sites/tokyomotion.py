...
def List(url, page=1):
    ...
    if not cards:
        ...
        for videopage, img, name, hd, duration in match:
            ...
            site.add_download_link(utils.cleantext(name), videopage, 'Playvid', img, name, duration=utils.cleantext(duration), quality=hd)
    else:
        for card in cards:
    ...
    if next_link:
        ...
        if next_href:
            ...
            site.add_dir('Next Page {}'.format(npage_label), nurl, 'List', site.img_next)
    utils.eod()
