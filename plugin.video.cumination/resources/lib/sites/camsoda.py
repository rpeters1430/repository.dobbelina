...
def Main():
    ...
    site.add_dir(
        "[COLOR red]Refresh Camsoda images[/COLOR]",
        "",
        "clean_database",
        "",
        Folder=False,
    ...
    if female:
        site.add_dir(
            ...
            site.url + "/api/v1/browse/react?gender-hide=c,t,m",
            "List",
            "",
            1,
        )
        site.add_dir(
            "[COLOR hotpink]North America - Female[/COLOR]",
            site.url + "/api/v1/browse/react/girls/region/usa?gender-hide=c,t,m",
            "List",
            "",
            1,
        )
        site.add_dir(
            "[COLOR hotpink]Central/South America - Female[/COLOR]",
            site.url
            + "/api/v1/browse/react/girls/region/central-south-america?gender-hide=c,t,m",
            "List",
            "",
            1,
        )
        site.add_dir(
            "[COLOR hotpink]Europe - Female[/COLOR]",
            site.url + "/api/v1/browse/react/girls/region/europe?gender-hide=c,t,m",
            "List",
            "",
            1,
        )
        site.add_dir(
            "[COLOR hotpink]Eastern Europe - Female[/COLOR]",
            site.url
            + "/api/v1/browse/react/girls/region/eastern-europe?gender-hide=c,t,m",
            "List",
            "",
            1,
        )
        site.add_dir(
            "[COLOR hotpink]Asia - Female[/COLOR]",
            site.url + "/api/v1/browse/react/girls/region/asia?gender-hide=c,t,m",
            "List",
            "",
            1,
        )
    if couple:
        site.add_dir(
            ...
            site.url + "/api/v1/browse/react?gender-hide=f,t,m",
            "List",
            "",
            1,
        )
    if male:
        site.add_dir(
            ...
            site.url + "/api/v1/browse/react?gender-hide=c,t,f",
            "List",
            "",
            1,
        )
    if trans:
        site.add_dir(
            "[COLOR violet]Transexual[/COLOR]",
            site.url + "/api/v1/browse/react?gender-hide=c,f,m",
            "List",
            "",
            1,
        )
...
def List(url, page):
    ...
    for camgirl in camgirls:
        ...
        subject += "[CR][CR][COLOR deeppink] Viewers: [/COLOR]{}[CR]".format(
            camgirl.get("connectionCount")
        )
        if camgirl.get("status"):
            subject += "[COLOR deeppink] Status: [/COLOR]{}[CR]".format(
                camgirl.get("status")
            )
        ...
        with conn:
            ...
            for row in lst:
                ...
                try:
                    ...
                    utils.kodilog(
                        "@@@@Cumination: Silent failure in camsoda: " + str(e)
                    )
            conn.execute(
                "DELETE FROM texture WHERE url LIKE ?;", ("%" + ".camsoda.com" + "%",)
            )
            if showdialog:
                utils.notify("Finished", "Camsoda images cleared")
    except Exception as e:
        utils.kodilog("@@@@Cumination: Silent failure in camsoda: " + str(e))
