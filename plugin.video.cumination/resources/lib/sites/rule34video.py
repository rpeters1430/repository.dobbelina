    ...
    if pager_next:
        ...
        if block_id and params_raw:
            ...
            if active_page:
                ...
                if active_link:
                    params_match = re.search(r'from(?:_albums)?:(\d+)', utils.safe_get_attr(active_link, 'data-parameters'))
                    if params_match:
            ...
            nextpg = '{0}?mode=async&function=get_block&block_id={1}&{2}'.format(url.split('?')[0], block_id, params)
        ...
        if params:
            ...
            if from_match:
                ...
                if '&from=' in url:
                    ...
                    nextpg = url
                nextpg = re.sub(r'&_=\d+', '&from={0}&_={1}'.format(from_value, int(time.time() * 1000)), nextpg)
