...
def Main():
    site.add_dir(
        ...
        site.url
        + "categories/?mode=async&function=get_block&block_id=list_categories_categories_list&sort_by=title&_=",
        "Categories",
        site.img_cat,
    )
    site.add_dir(
    ...
    site.add_dir(
        ...
        site.url
        + "playlists/?mode=async&function=get_block&block_id=list_playlists_common_playlists_list&sort_by=last_content_date&_=",
        "Categories",
        site.img_cat,
    )
    site.add_dir(
...
def List(url):
    if "?" not in url and ("/categories/" in url or "/models/" in url):
        url = (
            url
            + "?mode=async&function=get_block&block_id=list_videos_common_videos_list&sort_by=post_date&_="
            + str(1000000000000 + randint(0, 999999999999))
        )
    ...
    if "/playlists/" in url:
        ...
        for item in items:
            try:
                ...
                if (
                    "/categories/" in videourl
                    or "/models/" in videourl
                    or "/playlists/" in videourl
                ):
                    continue
                ...
                site.add_download_link(
                    name, videourl, "Playvid", img, name, contextm=cm
                )
        ...
        for item in items:
            try:
                ...
                if duration:
                    display_name = (
                        display_name + " [COLOR yellow]" + duration + "[/COLOR]"
                    )

                site.add_download_link(
                    display_name, videourl, "Playvid", img, name, contextm=cm
                )
            except Exception as e:
        ...
        if current_page and next_link:
            try:
                ...
                if block_id:
                    ...
                    cm_page = (
                        utils.addon_sys
                        + "?mode=whoreshub.GotoPage"
                        + "&url="
                        + urllib_parse.quote_plus(nurl)
                        + "&np="
                        + str(npage)
                        + "&listmode=whoreshub.List"
                    )
                    cm = [
                        (
                ...
                if nextp_match:
                    ...
                    if "from={0:02d}".format(pg) in url:
                        next_page = url.replace(
                            "from={0:02d}".format(pg), "from={0:02d}".format(int(np))
                        )
                    else:
...
def Playlist(url):
    ...
    for item in items:
        try:
            ...
            if name:
                name = utils.cleantext(name) + "[COLOR deeppink] {0}[/COLOR]".format(
                    count
                )
...
def WHLogin(logged=True):
    ...
    if not whlogged or "false" in whlogged:
        ...
        if whuser == "":
            ...
            whpass = getinput(
                default=whpass, heading="Input your Whorehub password", hidden=True
            )
