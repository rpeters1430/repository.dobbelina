...
def read_symbol(table, index, br):
    ...
    x_bits = br.read_bits(
        16, 0
    )  # The C reference version assumes 15 is the max needed and uses 16 in this function
    ...
    while (symbol < num_symbols) and (space > 0):
        ...
        if code_len < kCodeLengthRepeatCode:
            ...
            if symbol + repeat_delta > num_symbols:
                raise Exception(
                    "[read_huffman_code_lengths] symbol + repeat_delta > num_symbols"
                )
...
def read_huffman_code(alphabet_size, tables, table, br):
    ...
    if simple_code_or_skip == 1:
        ...
        if num_symbols == 2:
            ...
            if (
                symbols[0] == symbols[1]
                or symbols[0] == symbols[2]
                or symbols[0] == symbols[3]
                or symbols[1] == symbols[2]
                or symbols[1] == symbols[3]
                or symbols[2] == symbols[3]
            ):
                raise Exception("[read_huffman_code] invalid symbols")
            if br.read_bits(1):
    ...
    table_size = brotli_build_huffman_table(
...
class HuffmanTreeGroup:
    def __init__(self, alphabet_size, num_huff_trees):
        ...
        self.codes = [0] * (
            num_huff_trees
            + num_huff_trees * kMaxHuffmanTableSize[(alphabet_size + 31) >> 5]
        )
    ...
    def decode(self, br):
        ...
        for i in range(0, self.num_huff_trees):
            ...
            table_size = read_huffman_code(
                self.alphabet_size, self.codes, next_entry, br
            )
...
def decode_block_type(
    max_block_type, trees, tree_type, block_types, ring_buffers, indexes, br
):
    ...
    while not input_end:
        ...
        if is_uncompressed:
            copy_uncompressed_block_to_output(
                meta_block_remaining_len, pos, output_buffer, br
            )
        ...
        while meta_block_remaining_len > 0:
            ...
            for j in range(0, insert_length):
                ...
                prev_byte1 = read_symbol(
                    hgroup[0].codes, hgroup[0].huff_trees[literal_huff_tree_index], br
                )
            ...
            if distance_code < 0:
                ...
                distance_code = read_symbol(
                    hgroup[2].codes, hgroup[2].huff_trees[dist_huff_tree_index], br
                )
            ...
            if distance > max_distance:
                if (
                    ...
                    if transform_idx < kNumTransforms:
                        length = Transform.transformDictionaryWord(
                            output_buffer, copy_dst, offset, copy_length, transform_idx
                        )
                        copy_dst += length
                        pos += length
                        ...
                        raise Exception(
                            "Invalid backward reference. pos: %s distance: %s len: %s bytes left: %s"
                ...
                for j in range(
                    0, copy_length
                ):  # don't try to optimize with a slice. source and dest may overlap
