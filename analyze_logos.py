...
SITES_DIR = os.path.join(REPO_ROOT, "plugin.video.cumination", "resources", "lib", "sites")
...
def get_all_sites():
    ...
    sites = {}
    site_pattern = re.compile(r"site\s*=\s*AdultSite\s*\(\s*['\"]([^'\"]+)['\"].*?['\"]([^'\"]+\.(png|jpg|gif|PNG))['\"]", re.DOTALL)

    for filename in os.listdir(SITES_DIR):
        if filename.endswith('.py') and filename != '__init__.py' and filename != 'soup_spec.py':
                ...
                with open(filepath, 'r', encoding='utf-8') as f:
                    ...
                    if match:
                        ...
                        sites[site_id] = {
                            'module': filename,
                            'logo': logo_file,
                            'logo_exists': os.path.exists(os.path.join(IMAGES_DIR, logo_file))
                        }
...
def analyze():
    ...
    orphaned = [logo for logo in logos.keys() if logo not in site_logos and not logo.startswith('cum-')]
