...
def main():
    ...
    for filename in sorted(os.listdir(IMAGES_DIR)):
        if filename.lower().endswith(('.png', '.jpg', '.gif')) and not filename.startswith('cum-'):
    ...
    if widths:
        print(f"  Width:  min={min(widths)}px, max={max(widths)}px, avg={sum(widths)//len(widths)}px")
        print(f"  Height: min={min(heights)}px, max={max(heights)}px, avg={sum(heights)//len(heights)}px")
        print(f"  Size:   min={min(sizes):.1f}KB, max={max(sizes):.1f}KB, avg={sum(sizes)/len(sizes):.1f}KB")
