...
def test_list_parses_video_items(monkeypatch):
    ...
    def fake_add_download_link(name, url, mode, iconimage, desc="", fanart=None, quality='', **kwargs):
        downloads.append({
            'name': name,
            'url': url,
            'mode': mode,
...
def test_playvid_extracts_episode_id(monkeypatch):
    ...
    monkeypatch.setattr(hentaistream.utils, "prefquality", lambda *a, **k: '/720/manifest.mpd')
    ...
    # Call Playvid
    hentaistream.Playvid('https://hstream.moe/hentai/sample-anime-1', 'Sample Anime Episode 1')
