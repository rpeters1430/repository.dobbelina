...
class TestFavoritesBasicCRUD:
    ...
    def test_addFav(self, favorites_module, temp_db):
        ...
        c = conn.cursor()
        c.execute(
            "SELECT * FROM favorites WHERE url = ?", ("https://example.com/video1",)
        )
        result = c.fetchone()
        conn.close()
...
class TestFavoritesDuplicates:
    ...
    def test_delete_duplicates(self, favorites_module, temp_db):
        ...
        c = conn.cursor()
        c.execute(
            "SELECT COUNT(*) FROM favorites WHERE url = ?", ("https://example.com/dup",)
        )
        count = c.fetchone()[0]
        conn.close()
        assert count == 2
...
class TestCustomSites:
    ...
    def test_enabled_custom_sites(self, favorites_module, temp_db):
        ...
        c = conn.cursor()
        c.execute(
            "UPDATE custom_sites SET enabled = 1 WHERE name = ?", ("enabled_site",)
        )
        conn.commit()
        conn.close()
    ...
    def test_disable_custom_site_by_module(self, favorites_module, temp_db):
        ...
        c = conn.cursor()
        c.execute(
            "UPDATE custom_sites SET enabled = 1 WHERE module_file = ?",
            ("custom_disable",),
        )
        conn.commit()
        conn.close()
        ...
        c = conn.cursor()
        c.execute(
            "SELECT enabled FROM custom_sites WHERE module_file = ?",
            ("custom_disable",),
        )
...
class TestGetSiteName:
    ...
    def test_get_site_name_by_name(self, favorites_module):
        ...
        result = favorites_module.get_site_name(
            "example", "notmatching.com", adultsites
        )
