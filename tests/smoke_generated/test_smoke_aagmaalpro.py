"""Smoke tests for aagmaalpro site"""
...
def site_module():
    """Get site module"""
    return importlib.import_module("resources.lib.sites.aagmaalpro")
...
def _dispatch_preferred_listing(site_object, captured_items):
    ...
    for mode in URL_Dispatcher.func_registry.keys():
        if mode.startswith("aagmaalpro.") and 'list' in mode.lower():
            list_mode = mode
            break
...
class TestSearch:
    ...
    def test_search_returns_results(self, site_object, captured_items, mock_gethtml):
        ...
        search_mode = None
        for mode in URL_Dispatcher.func_registry.keys():
            if mode.startswith("aagmaalpro.") and 'search' in mode.lower():
                search_mode = mode
                break
