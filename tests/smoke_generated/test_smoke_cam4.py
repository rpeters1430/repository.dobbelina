...
def site_module():
    """Get site module"""
    return importlib.import_module("resources.lib.sites.cam4")
...
def _dispatch_preferred_listing(site_object, captured_items):
    ...
    for mode in URL_Dispatcher.func_registry.keys():
        if mode.startswith("cam4.") and 'list' in mode.lower():
            list_mode = mode
            break
...
# Note: cam4 is a webcam/live site
