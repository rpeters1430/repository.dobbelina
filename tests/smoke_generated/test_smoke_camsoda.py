...
def site_module():
    """Get site module"""
    return importlib.import_module("resources.lib.sites.camsoda")
...
def _dispatch_preferred_listing(site_object, captured_items):
    ...
    for mode in URL_Dispatcher.func_registry.keys():
        if mode.startswith("camsoda.") and 'list' in mode.lower():
            list_mode = mode
            break
...
# Note: camsoda is a webcam/live site
