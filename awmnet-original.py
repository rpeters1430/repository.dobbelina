...
def List(url):
    ...
    match = re.compile(
        r'class="item-link.+?href="([^"]+)".+?title="([^"]+)".+?src="([^"]+)".+?float-right(.*?)class="item-rating.+?text-xsm"></i>([^<]+)</a>',
    ...
    for videourl, name, thumb, info, provider in match:
        ...
        name = "[COLOR yellow][{}][/COLOR] {}".format(
            provider.strip(), utils.cleantext(name)
        )
    ...
    p = re.search(
        r'href="([^"]+)"[^>]+?label="Next\s*Page"', listhtml, re.DOTALL | re.IGNORECASE
    )
    if p:
        purl = siteurl[:-1] + p.group(1).replace("&amp;", "&")
        curr_pg = re.findall(
            r'label="Current\s*Page.+?(\d+)', listhtml, re.DOTALL | re.IGNORECASE
        )[0]
        site.add_dir(
...
def Tags(url):
    ...
    match = re.compile(
        r'<li\s*class="category".+?href="([^"]+)"><span class="category-title">([^<]+).+?>([\d\.km]+)<',
        re.DOTALL | re.IGNORECASE,
    ).findall(cathtml)
    for catpage, name, videos in match:
        name = (
            utils.cleantext(name) + " [COLOR deeppink](" + videos + " videos)[/COLOR]"
        )
        site.add_dir(
...
def Categories(url):
    ...
    cathtml = utils.getHtml(url, siteurl)
    match = re.compile(
        r'class="card\s*group.+?href="([^"]+)"\s*title="([^"]+)".+?src="([^"]+).+?>([\d\.km]+)<',
        re.DOTALL | re.IGNORECASE,
    ).findall(cathtml)
