...
def diagnose_listing_page(url="https://peachurnet.com/en"):
    ...
    for i, link in enumerate(video_links[:3], 1):
        ...
        print(f"\nSearching for title:")
        for selector in ['.title', '.video-title', 'h3', 'h2', 'h4', 'p', 'span', 'div']:
...
def diagnose_video_page(url):
    ...
    m3u8_urls = re.findall(r'https?://[^\s"\'<>]+\.m3u8[^\s"\'<>]*', html, re.IGNORECASE)
...
if __name__ == "__main__":
    ...
    if args.video:
        ...
        try:
            response = requests.get("https://peachurnet.com/en", verify=False, timeout=10)
